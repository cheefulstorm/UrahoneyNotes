...
sa_flags参数可以设置自动重启标志SA_RESTART（位操作），设置SA_RESTART后，对于部分慢系统调用，若被捕获的信号打断执行，可以重启重新执行
...
若系统调用会由于等待硬件设备或者网络设备导致阻塞，这种阻塞可能是永久性的，一般是IO或者网络相关的系统调用，可以称为慢系统调用，**若慢系统调用在阻塞的时间段内进程接收到了信号，若进程未终止，带到信号处理完成后，慢系统调用被中断执行会返回-1，置errno为EINTR**

> 有些慢系统调用被信号中断具有重启功能，重新执行系统调用，但是有些就不具有这个功能，需要自行判断errno然后再次调用
